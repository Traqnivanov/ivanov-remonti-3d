...
Ivanov Unique не е еднократна проверка само преди Owner approval.

Той остава активен **през целия lifecycle на всяка задача**.

Ако по време на implementation, QA или review се открие материално по-добър механизъм/логика/последователност:
1. спира се само засегнатата работа;
2. сравнява се текущото одобрено решение с по-силната алтернатива;
3. описват се конкретната полза, dependencies, side effects и рискът от rework;
4. ако промяната засяга продуктово поведение, UX, architecture, quantity/price truth, security/privacy или service meaning — спира се за Owner decision;
5. старото решение остава активно, докато Owner не го замени;
6. при одобрена замяна старото се маркира superseded и source-of-truth се синхронизира преди продължаване.

**APPROVED / LOCKED = защитена текуща база, не забрана да бъде намерено по-добро решение.**

Този gate не разрешава самоволно разширяване на scope, рестарт на проекта или „подобрения“ без доказана реална полза.
